    ...
    <div style={{ fontFamily: "var(--body)", color: "var(--ink)", overflowX: "hidden" }}>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        @media(max-width:768px){.nav{padding:0 20px;height:76px;}.nav-name{font-size:16px;}.nav-links{display:none;}.nav-links.open{display:flex;flex-direction:column;position:fixed;top:76px;left:0;right:0;bottom:0;background:#FFFFFF;padding:16px 24px;gap:0;align-items:flex-start;z-index:9999;overflow-y:auto;border-top:3px solid var(--gold);box-shadow:0 8px 32px rgba(13,27,53,0.15);}.nav-links.open a{font-family:var(--serif);font-size:20px;font-weight:700;color:var(--ink)!important;padding:14px 0;border-bottom:1px solid var(--ink25);width:100%;display:block;text-decoration:none;}.nav-links.open li{width:100%;list-style:none;}.nav-links.open .nav-cta{font-family:var(--sans);font-size:15px;background:var(--blue);color:#fff!important;padding:14px 24px;border-radius:4px;margin-top:12px;text-align:center;justify-content:center;}.mob-btn{display:block;z-index:10000;position:relative;}.sp{padding:70px 24px;}.hero-inner{padding:100px 24px 70px;}.about-left,.about-right{padding:50px 24px;}.cta-sec,.stats-band,.footer{padding:70px 24px;}.footer-top{grid-template-columns:1fr;gap:36px;}.feat-grid{grid-template-columns:1fr;}.steps-grid{grid-template-columns:1fr;}.stats-band{grid-template-columns:repeat(2,1fr);}.stat-item{border-right:none;border-bottom:1px solid var(--ink25);}.pricing-grid{grid-template-columns:1fr;padding:24px 20px 60px;gap:28px;}.pricing-header{padding:16px 20px;flex-wrap:wrap;gap:12px;}.pricing-hero{padding:60px 20px 16px;}}
      `}</style>
